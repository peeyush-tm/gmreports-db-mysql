...
/*
	...
	*/
        ...
        DROP TABLE IF EXISTS temp_big_invoice_return_table;
        CREATE TEMPORARY TABLE temp_big_invoice_return_table AS
		SELECT 'SUBS_MONTHLY' AS CHARGED_RECORD_TYPE,@ACCOUNT_ID AS ACCOUNT_ID,@ACCOUNT_NAME AS ACCOUNT_NAME,@OPERATOR_ACCOUNT_ID AS OPERATOR_ACCOUNT_ID,
			...
			OVERAGE_CHARGES AS TOTAL_CHARGES,0 AS TOTAL_OVERAGE_CHARGES,ROAMING_CHARGES AS TOTAL_ROAMING_CHARGES,0 AS MANDATORY_COL_NULL
		...
		SELECT 'DATA_MONTHLY' AS CHARGED_RECORD_TYPE,@ACCOUNT_ID AS ACCOUNT_ID,@ACCOUNT_NAME AS ACCOUNT_NAME,@OPERATOR_ACCOUNT_ID AS OPERATOR_ACCOUNT_ID,
			...
			OVERAGE_CHARGES AS TOTAL_CHARGES,OVERAGE_CHARGES AS TOTAL_OVERAGE_CHARGES,ROAMING_CHARGES AS TOTAL_ROAMING_CHARGES,0 AS MANDATORY_COL_NULL
		...
		-- return the data
		SELECT temp_big_invoice_return_table.*,
		CASE WHEN ACCOUNT_ID IS NULL OR ACCOUNT_NAME IS NULL OR OPERATOR_ACCOUNT_ID IS NULL OR
		ACCOUNT_TAX_ID IS NULL OR BILLING_CYCLE IS NULL OR BILLING_MONTH OR BILLING_YEAR IS NULL OR
		INVOICE_ID IS NULL OR CURRENCY_CODE IS NULL OR BILLABLE_FLAG IS NULL
		THEN 1 ELSE 0 END AS MANDATORY_COL_NULL
		FROM temp_big_invoice_return_table
		ORDER BY MANDATORY_COL_NULL DESC;
...
-- Dumping structure for procedure gmsa_reports.gm_get_mno_account_id
...
/*
	...
	*/
    ...
    WHERE (cdr_sms_details.SENT_TIME BETWEEN in_start_date AND in_end_date)  
		AND UPPER(SMS_STATUS) = 'SUCCESS' GROUP BY SOURCE,DESTINATION;
	...
	DROP TABLE IF EXISTS temp_recurring_table;
    CREATE TEMPORARY TABLE temp_recurring_table
	...
	SELECT report_metadata.ICCID,   
            ...
            COALESCE(temp_cdr_sms_details.SUCCESSFULL_SMS,0) AS SUCCESSFULL_SMS,        
...
ALTER TABLE report_metadata ADD COLUMN ENT_ACCOUNT_NAME varchar(256);
